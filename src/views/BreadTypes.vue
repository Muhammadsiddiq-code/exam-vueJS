<!-- <template>
  <div class="novvoylar-container">
    <Sidebar :menu-items="menuItems" :active-menu="'novvoylar'" :username="username" :user-role="userRole"
      @logout="handleLogout" @navigate="$emit('navigate', $event)" />
            ...
            <tr v-for="(novvoy, index) in novvoylar" :key="novvoy.id">
              ...
              <td>
                <div class="action-buttons">
                  <button @click="viewNovvoy(novvoy)" class="action-btn view-btn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="16" height="16">
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                      <circle cx="12" cy="12" r="3"></circle>
                    </svg>
                  </button>
                  <button @click="editNovvoy(novvoy)" class="action-btn edit-btn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="16" height="16">
                      <path d="M12 20h9"></path>
                      <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
                    </svg>
                  </button>
                  <button @click="deleteNovvoy(novvoy)" class="action-btn delete-btn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="16" height="16">
    ...
    <div v-if="showCreateModal" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          ...
          <button style="width: 50px; height: 40px; all: unset; margin: 0; padding: 0" @click="closeModal"
            class="close-button">
            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="red"
        ...
        <form @submit.prevent="createNovvoy" class="modal-form">
          <div class="group">
            <div class="form-group">
              ...
              <input id="novvoyName" type="text" v-model="newNovvoy.name" placeholder="Novvoyni nomini kiriting"
                required />
            </div>

            <div class="form-group">
              <label for="novvoyPassword">Parol</label>
              <input id="novvoyPassword" type="password" v-model="newNovvoy.password"
                placeholder="Novvoy parolini kiriting" required />
            </div>
          </div>
    ...
    <div class="main-content">
      ...
      <div class="content-header">
        <div class="header-left">
          <div>
            <h1 class="page-title">Nonlar</h1>
            <div class="breadcrumb">
              <span>Dashboard</span>
              <ChevronRight class="breadcrumb-separator" />
              <span class="breadcrumb-current">Nonlar</span>
            </div>
          </div>
        </div>
        <div class="header-right">
          <div class="header-actions">
            ...
            <div class="search-box">
              ...
              <input type="text" placeholder="Nonvoy qidirish..." v-model="searchQuery" @input="handleSearch"
                class="search-input" />
      ...
      <div class="stats-grid">
        <div class="stat-card" @click="filterByStatus('all')">
          ...
          <div class="stat-content">
            ...
            <p class="stat-label">Jami Nonlar</p>
        ...
        <div class="stat-card" @click="filterByStatus('active')">
          ...
          <div class="stat-content">
            ...
            <p class="stat-label">Faol Nonlar</p>
      ...
      <div class="table-container">
        ...
        <div class="table-header">
          <div class="table-title">
            <h3>Nonlar ro'yxati</h3>
            <span class="table-count">{{ filteredWorkers.length }} ta nonvoy</span>
          </div>
          <div class="table-actions">
            <button class="bulk-action-btn" :disabled="selectedWorkers.length === 0"
        ...
        <div v-else-if="viewMode === 'table'" class="table-wrapper">
          <table class="workers-table">
            <thead>
              <tr>
                <th class="checkbox-column">
                  <input type="checkbox" :checked="allSelected" @change="toggleSelectAll" class="table-checkbox" />
                </th>
            ...
            <tbody>
              <tr v-for="(worker, index) in paginatedWorkers" :key="worker.id" class="table-row"
                :class="{ selected: selectedWorkers.includes(worker.id) }">
                <td>
                  <input type="checkbox" :checked="selectedWorkers.includes(worker.id)"
                    @change="toggleSelect(worker.id)" class="table-checkbox" />
                </td>
        ...
        <div v-else class="cards-grid">
          <div v-for="worker in paginatedWorkers" :key="worker.id" class="worker-card"
            ...
            <div class="card-header">
              <input type="checkbox" :checked="selectedWorkers.includes(worker.id)" @change="toggleSelect(worker.id)"
                class="card-checkbox" />
        ...
        <div v-if="totalPages > 1" class="pagination">
          ...
          <div class="pagination-numbers">
            <button v-for="page in visiblePages" :key="page"
              :class="['pagination-number', { active: page === currentPage }]" @click="goToPage(page)"
              :disabled="page === '...'">
              {{ page }}
            </button>
          </div>
        ...
        <form @submit.prevent="saveWorker" class="modal-form">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="text" class="form-input" v-model="currentWorker.name" placeholder="Nonvoy nomini kiriting"
                required :class="{ error: formErrors.name }" />
              <span v-if="formErrors.name" class="form-error">{{ formErrors.name }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">Telefon raqami *</label>
              <input type="tel" class="form-input" v-model="currentWorker.phone" placeholder="+998 90 123 45 67"
                required :class="{ error: formErrors.phone }" @input="formatPhoneInput" />
              <span v-if="formErrors.phone" class="form-error">{{ formErrors.phone }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">Passport seriya va raqami *</label>
              <input type="text" class="form-input" v-model="currentWorker.passport" placeholder="AA 1234567" required
                :class="{ error: formErrors.passport }" @input="formatPassportInput" />
              <span v-if="formErrors.passport" class="form-error">{{ formErrors.passport }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">Tug'ilgan sana *</label>
              <input type="date" class="form-input" v-model="currentWorker.birthDate" required
                :class="{ error: formErrors.birthDate }" />
              <span v-if="formErrors.birthDate" class="form-error">{{ formErrors.birthDate }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">Ish boshlagan sana *</label>
              <input type="date" class="form-input" v-model="currentWorker.startDate" required
                :class="{ error: formErrors.startDate }" />
              <span v-if="formErrors.startDate" class="form-error">{{ formErrors.startDate }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">Oylik maosh *</label>
              <input type="number" class="form-input" v-model="currentWorker.salary" placeholder="0" required min="0"
          ...
          <div class="form-group">
            <label class="form-label">Manzil *</label>
            <textarea class="form-textarea" v-model="currentWorker.address" placeholder="To'liq manzilni kiriting"
              rows="3" required :class="{ error: formErrors.address }"></textarea>
            <span v-if="formErrors.address" class="form-error">{{ formErrors.address }}</span>
          </div>
          <div class="form-group">
            <label class="form-label">Qo'shimcha ma'lumot</label>
            <textarea class="form-textarea" v-model="currentWorker.notes" placeholder="Qo'shimcha ma'lumotlar..."
              rows="3"></textarea>
          </div>
          <div class="form-actions">
              ...
              <div class="detail-items">
                <div class="detail-item">
                  ...
                  <div>
                    ...
                    <a :href="`tel:${viewingWorker.phone}`" class="detail-value">{{ formatPhone(viewingWorker.phone)
                      }}</a>
                  </div>
                </div>
                <div class="detail-item">
...
.main-content {
  flex: 1;
  margin-left: 280px;
  /* O'zingizning sidebar kengligiga qarab o'zgartiring */
  padding: 24px;
  overflow-x: auto;
}
